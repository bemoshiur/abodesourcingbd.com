...
export function factoriesEntry(site: SiteInfo, pc: PageMeta): Entry {
  return {
    kind: "factories",
    path: "/factories/",
    title: pick(pc.metaTitle, "Compliant Garment Factories in Bangladesh & India"),
    description: pick(
      pc.metaDescription,
...
export function factoryEntry(site: SiteInfo, f: FactoryView): Entry {
  ...
  const long = `${f.name}: Partner Factory in ${where}`;
  return {
    kind: "factory",
    path: `/factories/${f.slug}/`,
    title: pick(f.seo.metaTitle, long.length <= 60 ? long : `${f.name}: Factory in ${where}`),
    description: pick(
